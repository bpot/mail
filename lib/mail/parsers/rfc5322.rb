...
module Mail
  module RFC5322
    ...
    def _nt_utext
      start_index = index
      if node_cache[:utext].has_key?(index)
        cached = node_cache[:utext][index]
        if cached
          cached = SyntaxNode.new(input, index...(index + 1)) if cached == true
          @index = cached.interval.end
        end
      ...
      node_cache[:utext][start_index] = r0

      r0
    end

    def _nt_received_token
      start_index = index
      if node_cache[:received_token].has_key?(index)
        cached = node_cache[:received_token][index]
        if cached
          cached = SyntaxNode.new(input, index...(index + 1)) if cached == true
          @index = cached.interval.end
        end
      ...
      node_cache[:received_token][start_index] = r0
  ...
  class RFC5322Parser < Treetop::Runtime::CompiledParser
    include RFC5322
  end
