...
module Mail
  module RFC5322Obsolete
    ...
    def _nt_obs_qp
      ...
      if r1
        i2 = index
        if has_terminal?('\G[\\x00]', true, index)
          r3 = true
          @index += 1
        else
          r3 = nil
    ...
    def _nt_obs_ctext
      start_index = index
      if node_cache[:obs_ctext].has_key?(index)
        cached = node_cache[:obs_ctext][index]
        if cached
          cached = SyntaxNode.new(input, index...(index + 1)) if cached == true
          @index = cached.interval.end
        end
      ...
      r0 = _nt_obs_NO_WS_CTL

      node_cache[:obs_ctext][start_index] = r0

      r0
    end

    def _nt_obs_NO_WS_CTL
      start_index = index
      if node_cache[:obs_NO_WS_CTL].has_key?(index)
        cached = node_cache[:obs_NO_WS_CTL][index]
        if cached
          cached = SyntaxNode.new(input, index...(index + 1)) if cached == true
          @index = cached.interval.end
        end
      ...
      if r1
        ...
        if has_terminal?('\G[\\x0b]', true, index)
          r2 = true
          @index += 1
        else
          r2 = nil
        ...
        if r2
          ...
          if has_terminal?('\G[\\x0c]', true, index)
            r3 = true
            @index += 1
          else
            r3 = nil
      ...
      node_cache[:obs_NO_WS_CTL][start_index] = r0
    ...
    def _nt_obs_dtext
      start_index = index
      if node_cache[:obs_dtext].has_key?(index)
        cached = node_cache[:obs_dtext][index]
        if cached
          cached = SyntaxNode.new(input, index...(index + 1)) if cached == true
          @index = cached.interval.end
        end
      ...
      node_cache[:obs_dtext][start_index] = r0
  ...
  class RFC5322ObsoleteParser < Treetop::Runtime::CompiledParser
