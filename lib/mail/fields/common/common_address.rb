...
module Mail
  module CommonAddress # :nodoc:
    ...
    # Allows you to iterate through each address object in the syntax address_list
    def each
    ...
    def do_encode(field_name)
      ...
      address_array = address_list.addresses.reject { |a| group_addresses.include?(a.encoded) }.compact.map { |a| a.encoded }
    ...
    def do_decode
      ...
      address_array = address_list.addresses.reject { |a| group_addresses.include?(a.decoded) }.map { |a| a.decoded }
      address_text  = address_array.join(", ")
